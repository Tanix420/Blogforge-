...
export default function AdminArticles() {
 ...
 useEffect(() => {
  fetch('/api/admin/articles', { cache: 'no-store', credentials: 'include' })
   .then((r) => r.ok ? r.json() : Promise.reject(r.status))
   .then((data) => {
    const list = (Array.isArray(data) ? data : (data.articles ?? [])).map((a: any) => ({
     slug: a.slug,
 ...
 const toggleStatus = async (slug: string) => {
  ...
  setRows((prev) => prev.map((r) => (r.slug === slug ? { ...r, status: next } : r)));
 };
 ...
 return (
  <div className="space-y-6">
   ...
   <FadeUp delay={0.12} className="card overflow-hidden">
    {loading ? (
     <div className="text-sm py-8 text-center" style={{ color: 'var(--ink-muted)' }}>Loading articles…</div>
    ) : (
      ...
      <tbody>
       {filtered.map((row) => (
        <tr key={row.slug} className="border-b last:border-0" style={{ borderColor: 'var(--border-subtle)' }}>
         ...
         <td className="px-4 py-3 font-mono" style={{ color: 'var(--accent-strong)' }}>SEO {row.seoScore}</td>
         <td className="px-4 py-3 text-right">
          <div className="inline-flex items-center gap-2">
           <button className="btn btn-ghost text-xs" onClick={() => toggleStatus(row.slug)}>
            {row.status === 'published' ? 'Unpublish' : 'Publish'}
           </button>
           <button className="btn btn-ghost text-xs" style={{ color: 'var(--danger)' }}><Trash2 size={14} /></button>
          </div>
         </td>
        </tr>
       ))}
       {!filtered.length && (
        <tr>
         <td colSpan={4} className="text-sm py-8 text-center" style={{ color: 'var(--ink-muted)' }}>No articles match your search.</td>
        </tr>
       )}
      </tbody>
     </table>
