...
export default function HomePage() {
 ...
 <h2 style={{ fontSize: 13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#6a7388', margin: '0 0 24px', textAlign: 'center' }}>Latest Articles</h2>
 ...
 <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 16, justifyContent: 'center' }}>
 ...
 <Image src={article.featuredImage} alt={article.title} fill style={{ objectFit: 'cover' }} sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" priority />
 ) : (
 ...
 position: 'absolute',
 inset: 0,
 background: 'linear-gradient(to top, rgba(0,0,0,0.88) 0%, rgba(0,0,0,0.35) 40%, rgba(0,0,0,0.05) 100%)',
 }} />
 ...
 <h3 style={{ fontSize: 17, fontWeight: 800, letterSpacing: '-0.02em', lineHeight: 1.3, margin: '0 0 6px', color: '#fff', textShadow: '0 1px 4px rgba(0,0,0,0.6)' }}>{article.title}</h3>
 <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.8)', lineHeight: 1.5, margin: 0, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{article.excerpt}</p>
 </div>
 </article>
 </Link>
 ))}
